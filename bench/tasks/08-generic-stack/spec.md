...
constructor wrapping the elements. Type parameters are written with a
leading quote, e.g. `type Option 'a = None | Some 'a`.

Implement:
- `push : 'a -> Stack 'a -> Stack 'a` — push an element on top
- `pop : Stack 'a -> Result String ('a, Stack 'a)` — remove and return the
  top element along with the remaining stack, or `Error "..."` if empty
- `peek : Stack 'a -> Result String 'a` — return the top element without
  removing it, or `Error "..."` if empty
