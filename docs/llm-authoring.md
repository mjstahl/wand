...
**A newline ends a statement at bracket depth 0.** Inside brackets it does
not. A definition ends at the end of its line.

**`let ... in` is an expression.** A top-level `let` is not. The two look
