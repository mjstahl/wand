...
let test_cons_expr () =
  ok "prepend one"    {|start 1 :: [2, 3]|}         "[1, 2, 3]";
  ok "chain cons"     {|start 1 :: 2 :: 3 :: []|}   "[1, 2, 3]";
  ok "cons onto empty" {|start 42 :: []|}            "[42]";
  ok "string cons"    {|start "a" :: ["b", "c"]|}   "[a, b, c]"

(* ── List patterns ───────────────────────────────────────────────────────── *)

let test_empty_pattern () =
  ok "match empty"
    {|start match [] with | [] -> "empty" | _ -> "non-empty"|}
    "empty";
  ok "match non-empty"
    {|start match [1] with | [] -> "empty" | _ -> "non-empty"|}
    "non-empty"

let test_exact_pattern () =
  ok "exact one element"
    {|start match [42] with | [x] -> x | _ -> 0|}
    "42";
  ok "exact two elements"
    {|start match [3, 4] with | [x, y] -> x + y | _ -> 0|}
    "7"

let test_cons_pattern () =
  ok "head"
    {|start match [1, 2, 3] with | [h :: _] -> h | [] -> 0|}
    "1";
  ok "tail"
    {|start match [1, 2, 3] with | [_ :: t] -> t | [] -> []|}
    "[2, 3]";
  ok "no match on empty"
    {|start match [] with | [_ :: _] -> "yes" | [] -> "no"|}
    "no"
...
let test_type_errors () =
  err "cons non-list rhs"    {|start 1 :: 2|};
  err "cons type mismatch"   {|start 1 :: ["a", "b"]|}
