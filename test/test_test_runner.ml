...
(* `t.eq` takes what the value should be first and the value second, as
   every wand function takes its subject last -- so `x |> t.eq expected`
   reads as a pipeline like anything else. *)
