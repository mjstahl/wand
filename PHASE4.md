...
**Status:** P4.1, P4.4 and P4.5 done · **Goal:** a wand binary that works where it is put,
...
## P4.5 — `setup-wand` action · done

`mjstahl/setup-wand`, tagged `v1`. CI is the beachhead: a controlled
environment where per-repo tooling is already normal, and where a script can
be adopted one repository at a time.

A composite action, so there is nothing to build and nothing to trust past
what the file says. It works out the runner's platform, downloads the
matching archive, **verifies its `sha256`**, unpacks it into the tool cache
and adds it to `PATH`; a second run on the same runner reuses the cache.
Before returning it runs the binary from a directory that is not a wand tree
-- running anywhere is the whole claim the artifact makes, so a bad download
fails there rather than in someone's build.

**Its test suite is written in wand.** `test/test_install.wand` carries
`uses {Shell, FS.Read, FS.Write}` and checks that the binary runs, that the
stdlib came with it, that a decoy `stdlib/` changes nothing, that `wand t`
reports a type, and that a type error is reported rather than run. Writing
it found three things about the language that reading the reference had not:
`String.contains?` takes `needle s` and keeps its question mark,
`FS.temp_dir` is a `Resource` used through `with ... as`, and a script that
calls `List.length` without importing `List` does not typecheck. Each was a
guess that running the file corrected.

*Accept:* met, as a job rather than a sentence. The workflow's last job
checks out nothing, says `uses: mjstahl/setup-wand@v1`, writes a wand test
on the spot and runs `wand test`. Green on `ubuntu-latest`,
`ubuntu-24.04-arm` and `macos-15`.

### `wand --version`, which this asked for

An installer that pins a version could not ask the binary what it was, so it
had to trust the archive's name. `wand --version` prints `wand 0.1.1` -- one
line, no prose, because an installer compares it rather than reads it. `-V`
and `version` are aliases.

The number lives in `VERSION`, generated into a module by the rule pattern
the stdlib table already uses, and `make release` refuses to tag unless the
file agrees with the tag. That is what stops a binary claiming a version
nobody released. Generated by a small OCaml program rather than a shell
action, because the Linux binaries are built in Alpine, which has no `bash`.

`0.1.0` predates the flag and treats `--version` as a filename, so the
action takes pre-`0.1.1` releases at their word instead of failing them.
...
P4.1, P4.4 and P4.5 are done. `v0.1.1` is published for four platforms and
`mjstahl/setup-wand@v1` installs it. **The order changed**: P4.5 was taken
before P4.2 and P4.3, on the phase's own reasoning -- P4.5 calls CI the
beachhead, and in CI you install the tool, so a self-contained executable
buys nothing there. Against that, P4.2 got more expensive when the signing
experiment turned it into two mechanisms with an arm64 leg still unverified,
and `wand t` already reports an inferred effect row, so the audit story does
not wait on `--manifest`. **P4.2 and P4.3 are what remain**, in that order,
...
**Releasing is two halves that do not wait for each other.** A `v*` tag
makes CI build three archives; `make release VERSION=x.y.z` tags, pushes,
builds the fourth on an Intel Mac and attaches it. Both create the release
if it is absent. It stays a draft until `gh release edit <tag>
--draft=false`. Bump `VERSION` and commit it first -- `make release` refuses
a tag the file disagrees with.

**`setup-wand` lives in its own repository**, because an action is consumed
by its own tag. Its test suite is written in wand, so a language change that
breaks a script shows up there rather than in a user's workflow -- but only
when something runs it, and nothing in this repository does.
...
5. **Met.** Released binaries exist for Linux and macOS, installable in one
   line -- `v0.1.1`, four archives with checksums, and
   `mjstahl/setup-wand@v1` installs it in a workflow.
